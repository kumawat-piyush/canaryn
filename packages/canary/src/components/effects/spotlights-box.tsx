...
function Root({ highlightTop = '', highlightBottom = '', logo, children }: PageProps) {
  ...
  return (
    <div
      ...
      <div className="absolute inset-0 flex items-center justify-center">{logo && logo}</div>
      {children}
    </div>
  )
}
...
function Content({ children, className }: { children: React.ReactNode; className?: string }) {
  return <div className={twMerge('absolute bottom-0 w-full px-4 py-3', className)}>{children}</div>
}
