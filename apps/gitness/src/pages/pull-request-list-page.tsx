...
function PullRequestListPage() {
  ...
  const renderListContent = () => {
    ...
    if (!pullrequests?.content?.length) {
      if (query) {
        return (
          <NoSearchResults
  ...
  const pullRequestsExist = (pullrequests?.content?.length ?? 0) > 0
  ...
  return (
    <>
      <PaddingListLayout spaceTop={false}>
        ...
        {renderListContent()}
        <Spacer size={8} />
        {pullRequestsExist && <PageControls totalPages={pullrequests?.headers?.['total']} />}
      </PaddingListLayout>
    </>
  )
}
